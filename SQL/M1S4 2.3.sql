...
SELECT
  ...
  SUM(Total_amount_with_tax) OVER (PARTITION BY Country_region_code ORDER BY Month) AS Cumulative_Total,
  DENSE_RANK() OVER (PARTITION BY Country_region_code ORDER BY Total_amount_with_tax DESC) AS sales_rank
FROM
  MonthlySales
ORDER BY
Region,
